define([], function () {
	...
	return {
		...
		isOEM: function (enigma, fieldCode) {
			setMod = atob(fieldCode).replace('[', '{').replace(']', '}');
			//"CaseLeadTime","ProcessPathUniqueNo","UniqueSortedRowNo","ProcessPath","ActivityTypeID","ProcessStepFollowUpIdleTime"
			return enigma.evaluate('FAbs('
				+ atob('Q291bnQoezE8JEZpZWxkPQ==') + setMod
				+ atob('Pn1ESVNUSU5DVCAkRmllbGQpPQ==') + setMod.split('","').length + ')'
			)
		}
	}
