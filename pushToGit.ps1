...
$confirmation = Read-Host "Increase the version counter now? (y/n)"
if ($confirmation -like 'y*') {
    $qextFilename = '.\ext_guided_tour_2.qext'
    ...
    if ($result) {
        ...
        Write-Host "increased version number to $($versionNoStr) for next time"
        $qext.version = $versionNoStr
        $qext | ConvertTo-Json -depth 100 | Out-File $qextFilename -Encoding Utf8
    }
    else {
        Write-Host "Cannot increase the version number, it isn't"
    }
}